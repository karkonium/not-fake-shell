...
/**
 * This class is responsible for having the basic properties of a tree i.e, adding,removing,
 * extracting data and searching
 *
 */
public class Tree extends ParentTree implements Serializable, ITree {

  static final long serialVersionUID = 0;

  private static Tree dir;
   ...
   * @param depth How deep into recursion
   * @param treeText The text of the node
   */
  public String recursiveTree(String path, int depth, String treeText) {
    ...
    for (int i = 0; i < depth; i++) {
      treeText += "\t";
      tabList += "\t";
      // display.standardOutput("\t");
    }
    if (!path.equals("/")) {
      treeText += root.getName() + "\n";

      // display.standardOutputLine(root.getName());
    }
  ...
  public void addDirectoryNode(String path) {
    try {
      ...
      if (workingNode != null) {
        ...
        display.standardOutputLine(path + ": Improper path");
      }
    } catch (Exception e) {
      return;
    }
  ...
  public void removeNode(String path) {
    try {
      ...
      display.standardOutputLine("Cannot remove Node");
    }
  }
  ...
  // ensure that
  /**
   * This methods obtains the sub tree from the bigger tree
   * 
   * @param current Current working node
   * @param start The node of which to start
  ...
  private void getSubTree(DirectoryNode current, DirectoryNode start, ArrayList<String> paths) {
    ...
    for (Node childNode : startChildrenList) {
      if (current.obtainChildStringList().contains(childNode.getName())) {
        current.removeNodeFromList(childNode.getName());
      }
  ...
  /**
   * This method obtains the sub directory of a folder given a path
   * 
   * @param path path to sub directory
   * @return An arrayList of sub directories
   */
  ...
  /**
   * This method copies the sub Tree
   * 
   * @param oldPath Path where files/folders are
   * @param newPath Path of where files/folders should be
   * 
   */
